...
export function simulationRequest(plan:Plan){
 if(plan.chainId!==8453||plan.executable!==false||!/^0x[0-9a-f]{64}$/i.test(plan.blockHash)||!Number.isFinite(Date.parse(plan.expiresAt))||Date.parse(plan.expiresAt)<=Date.now())throw Error('Invalid or expired plan');
 const checks=probes(plan);
 return {jsonrpc:'2.0',id:1,method:'eth_simulateV1',params:[{blockStateCalls:[{calls:[...checks,...plan.calls.map(c=>({from:plan.wallet,to:c.to,value:c.value,data:c.data})),...checks]}],validation:false,traceTransfers:false,returnFullTransactions:false},plan.blockHash]};
}
export function analyzeBasketSimulation(plan:Plan,result:any){
 ...
 if(typeof result[0].parentHash!=='string'||result[0].parentHash.toLowerCase()!==plan.blockHash.toLowerCase())throw Error('Simulation parent block mismatch');
 ...
 return {status:'SEQUENCE_SIMULATED',planId:plan.planId,simulatedAt:new Date().toISOString(),expiresAt:plan.expiresAt,blockTag:plan.blockHash,receivedUsdc:formatUnits(delta,6),inputEffects,gasUsedRaw:gas.toString(),callGasUsedRaw,executable:false,atomicWalletBatch:'NOT_VERIFIED',totalFee:null,limitations:['Sequential calls use the quote block hash with validation=false; nonce and gas affordability are not verified.','Gas units exclude balance probes and Base L1 data fees; full transaction fees are not verified.','Only selected ERC-20 balances and router allowances are checked; other assets, native balance and token honesty are not verified.','This is not actual wallet execution or a broadcast. Prior permission transactions are not rolled back by a failed swap.']};
}
