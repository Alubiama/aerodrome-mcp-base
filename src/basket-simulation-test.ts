...
const plan:any={wallet:'0x0000000000000000000000000000000000000001',chainId:8453,executable:false,blockHash:'0x'+'a'.repeat(64),expiresAt:new Date(Date.now()+60000).toISOString(),planId:'test',minimumOutput:'2',tokens:[2,3].map(n=>({token:'0x'+String(n).padStart(40,'0'),amountRaw:'100',allowanceRaw:'0'})),calls:[{to:'0x0000000000000000000000000000000000000002',value:'0x0',data:'0x'}]};
const bal=(n:bigint)=>({status:'0x1',returnData:'0x'+n.toString(16).padStart(64,'0')});
const ok=()=>[{parentHash:plan.blockHash,calls:[bal(1000000n),bal(1000n),bal(0n),bal(1000n),bal(0n),{status:'0x1',gasUsed:'0x5208'},bal(3100000n),bal(900n),bal(0n),bal(900n),bal(0n)]}];
const req:any=simulationRequest(plan);assert.equal(req.method,'eth_simulateV1');assert.equal(req.params[0].blockStateCalls[0].calls.length,11);assert.equal(req.params[0].validation,false);assert.equal(req.params[0].blockStateCalls[0].stateOverrides,undefined);assert.equal(req.params[1],plan.blockHash);
...
assert.equal(analyzeBasketSimulation(one,[{parentHash:one.blockHash,calls:oneCalls}]).receivedUsdc,'2.1');
assert.throws(()=>analyzeBasketSimulation(plan,[{...ok()[0],parentHash:'0x'+'b'.repeat(64)}]),/parent block/);
assert.throws(()=>analyzeBasketSimulation(plan,[{...ok()[0],parentHash:undefined}]),/parent block/);
...
for(const patch of [{chainId:1},{executable:true},{blockHash:'pending'},{blockHash:'0x'+'0'.repeat(63)},{expiresAt:'invalid'},{expiresAt:'2000-01-01'}])assert.throws(()=>analyzeBasketSimulation({...plan,...patch},ok()));
