...
const BASE_USDC_META: TokenMeta = {
  ...
  symbol: "USDC",
  decimals: 6, decimalsSource: "CANONICAL"
};
...
export async function getTokenMeta(client: any, address: Address, symbolFallback?: string, blockNumber?: bigint): Promise<TokenMeta> {
  ...
  let decimals = 18;
  let decimalsSource: TokenMeta["decimalsSource"] = "ASSUMED";
  ...
  try {
    ...
    if (Number.isInteger(fetched) && fetched >= 0 && fetched <= 36) { decimals = fetched; decimalsSource = "ONCHAIN"; }
  } catch {
    // non-standard decimals() — 18 is a safer display fallback than failing the whole scan
  }

  const meta = { address, symbol, decimals, decimalsSource };
